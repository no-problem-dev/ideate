...
## Limit
- 出力は本スキルの管轄ディレクトリに限定する（デフォルト: `ideas/app-shaping/`）
...
**引数なし:**
1. seed-validate の出力ディレクトリ（デフォルト: `ideas/app-validation/`）から最新の `*.md` ファイルを読み込む

**日付指定（例: 2026-02-14）:**
1. seed-validate の出力ディレクトリ（デフォルト: `ideas/app-validation/`）内で該当日付を含むファイルを検索して読み込む
...
2. **ファイル生成**:
   結果を埋め込んで出力ディレクトリの `YYYY-MM-DD-{slug}.md`（デフォルト: `ideas/app-shaping/YYYY-MM-DD-{slug}.md`）に出力する
...
4. **git commit**:
   ```bash
   git add {output_dir}/YYYY-MM-DD-{slug}.md && git commit -m "ideas: Seed Shaping YYYY-MM-DD"
   ```
...
パス: 出力ディレクトリの `YYYY-MM-DD-{slug}.md`（デフォルト: `ideas/app-shaping/YYYY-MM-DD-{slug}.md`）
...
## Signal
### 完了条件
- [ ] 出力ディレクトリの `YYYY-MM-DD-{slug}.md`（デフォルト: `ideas/app-shaping/YYYY-MM-DD-{slug}.md`）が作成されている
